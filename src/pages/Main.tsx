...
function Main() {
  ...
  useEffect(() => {
    ...
    if (Math.floor(playedSeconds) === 10 && stage === 1) {
      setDisplayAdvertisementNumber(2);
      setPlaying(false);
    }
    if (Math.floor(playedSeconds) === 13 && stage === 2) {
      setDisplayAdvertisementNumber(3);
      setPlaying(false);
    }
    if (Math.floor(playedSeconds) === 15 && stage === 3) {
      setStage(8);
    }
  }, [playedSeconds, stage]);
      ...
      <div className="App">
        ...
        <div className="flex flex-col gap-4">
          ...
          >
            ...
            >
              {displayAdvertisementNumber === 1 ? (
                <Advertisement1 clearAdvertisement={clearAdvertisement(1)} />
              ) : (
                <></>
              )}
              {displayAdvertisementNumber === 2 ? (
                <Advertisement2 clearAdvertisement={clearAdvertisement(2)} />
              ) : (
                <></>
              )}
              {displayAdvertisementNumber === 3 ? (
                <Advertisement3 clearAdvertisement={clearAdvertisement(3)} />
              ) : (
                <></>
              )}
              {displayAdvertisementNumber === 4 ? (
                <Advertisement4 clearAdvertisement={clearAdvertisement(4)} />
              ) : (
                <></>
              )}
            </div>
